...
ax = fig.add_axes([0.22, 0.18, 0.75, 0.625])
t_1000, = ax.plot(machines_1000, throughput_1000, color=config.green_sequence_colors[0], label="1s", marker=config.dumbo_marker)
t_500, = ax.plot(machines_500, throughput_500, color=config.blue_sequence_colors[1], label="0.5s", marker=config.dumbo_marker)
t_300, = ax.plot(machines_300, throughput_300, color=config.blue_sequence_colors[2], label="0.3s", marker=config.dumbo_marker)
machines = [i for i in range(2, 6)] + machines_300 + [machines_300[-1]+1]
#t_obladi, = ax.plot(machines, [obladi_throughput] * len(machines), color=config.obladi_color, label="2 machines, 0.xs", linestyle='--')
machines = [1] + machines
t_oblix, = ax.plot(machines, [oblix_throughput] * len(machines), color=config.oblix_color, label="Oblix (1 machine)", linestyle=config.oblix_line)
ax.set_xlabel("Machines")
ax.set_ylabel("Throughput (reqs/sec)")
ax.set_yticks([0, 10e3, 20e3])
ax.set_xlim(5, 17.3)
...
leg = fig.legend([t_1000, t_500, t_300], ['1s', '0.5s', '0.3s'],
    ...
    title='Snoopy-Oblix:', title_fontsize=8, mode='expand')
leg.get_frame().set_linewidth(0)
custom_style.legend_title_left(leg)

leg = fig.legend([t_oblix], ['Baseline Oblix (1 machine)'],
    bbox_to_anchor=(0.025, 0.82, 0.95, .1), ncol=3, loc='lower left', fontsize=8, handlelength=1.25, borderpad=0.2,
    borderaxespad=0, labelspacing=0, columnspacing=0.5, handletextpad=0.25,
    mode='expand')
leg.get_frame().set_linewidth(0)

fig.patches.extend([plt.Rectangle((0.015,0.82), 0.97, 0.17,
