...
oe_result_t enclave_claims_verifier_server(
    oe_claim_t* claims,
    size_t claims_length,
    void* arg)
{
...
oe_result_t enclave_claims_verifier_client(
    oe_claim_t* claims,
    size_t claims_length,
    void* arg)
{
...
int verify_callback(int preverify_ok, X509_STORE_CTX* ctx, bool isServer)
{
	...
	if (isServer) {
    	result = oe_verify_attestation_certificate_with_evidence(
        	der, der_len, enclave_claims_verifier_server, nullptr);
	} else {
	    result = oe_verify_attestation_certificate_with_evidence(
    	    der, der_len, enclave_claims_verifier_client, nullptr);
	}
    if (result != OE_OK)
    {
